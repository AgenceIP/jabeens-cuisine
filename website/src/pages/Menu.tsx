...
import { dineInMenuData, takeOutMenuData } from '@/data/menuData'
import type { MenuCategory } from '@/types'
import Footer from '@/components/layout/Footer'
import { useT, useLang } from '@/contexts/LanguageContext'

type MenuType = 'dineIn' | 'takeOut'
type LightboxItem = { image: string; name: string; description?: string; vegetarian?: boolean; price?: string }
...
function MenuItemCard({
  ...
  return (
    ...
    >
      {/* Text */}
      <div style={{ flex: 1, minWidth: 0, padding: '28px 0', paddingRight: item.image ? 32 : 0 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 10 }}>
          {item.vegetarian && (
            <span style={{ color: '#A8956A', fontSize: '0.65rem', flexShrink: 0 }}>✦</span>
          )}
          <p
            className="text-display text-text-primary"
            style={{ fontSize: '1.45rem', margin: 0, lineHeight: 1.2 }}
          >
            {item.name}
          </p>
        </div>
        {item.description && (
          <p
            ...
            style={{ fontSize: '0.88rem', lineHeight: 1.75, margin: '0 0 14px' }}
          >
            {item.description}
          </p>
        )}
        ...
        >
          <img
            ...
            onMouseLeave={e => {
              ...
              i.style.filter = 'brightness(0.88)'
            }}
          />
        </button>
      )}
        ...
        <div style={{ padding: '20px 24px 24px' }}>
          ...
          {item.description && (
            <p
              ...
              style={{ fontSize: '0.82rem', marginBottom: item.price ? 10 : 0 }}
            >
              {item.description}
            </p>
          )}
...
export default function Menu() {
  ...
  const [activeId, setActiveId] = useState('tapas')
  ...
  const PDF_MENUS = [
    {
      ...
      title: m.dineIn.toUpperCase(),
      subtitle: m.dineInSubtitle,
      pdf: '/menu/Restaurant MENU.pdf',
    },
    {
      type: 'takeOut' as MenuType,
      title: m.takeOut.toUpperCase(),
      subtitle: m.takeOutSubtitle,
      pdf: '/menu/TAKE OUT MENU.pdf',
    },
  ]
  const DRINKS_PDF = {
    title: m.drinks.toUpperCase(),
    subtitle: m.drinksSubtitle,
    pdf: '/menu/DrinkMenu.pdf',
  }

  const currentData = activeMenu === 'dineIn' ? dineInMenuData : takeOutMenuData

  const catLabel = (cat: MenuCategory) =>
    lang === 'fr' ? (cat.labelFr ?? cat.label) : cat.label
  const catSublabel = (cat: MenuCategory) =>
    lang === 'fr' ? (cat.sublabelFr ?? cat.sublabel) : cat.sublabel

  useEffect(() => {
    if (currentData.length > 0) setActiveId(currentData[0].id)
  }, [activeMenu])
  ...
  return (
    <>
      {/* Hero */}
      <div
        className="pt-32 pb-12 px-8 md:px-16 text-center"
              ...
              {currentData.map(cat => (
                ...
                >
                  ...
                  {cat.sublabel && (
                    <span
                      className="block"
                      style={{ fontSize: '0.52rem', opacity: 0.55, fontWeight: 400 }}
                    >
                      {catSublabel(cat)}
                    </span>
                  )}
            ...
            {currentData.map(category => (
              <section key={category.id} id={category.id} className="mb-10 md:mb-20">
                ...
                <div className="mb-2 md:mb-4">
                  <div
                    className="md:hidden flex flex-col items-center text-center py-4"
                  ...
                  >
                    ...
                    {category.sublabel && (
                      <p
                        className="text-text-muted font-light mt-1"
                  ...
                  <div className="hidden md:block pb-4" style={{ borderBottom: '1px solid #2A2A2A' }}>
                ...
                {category.items.map((item, i) => (
                  <MenuItemCard key={i} item={item} onImageClick={setLightbox} />
                ))}
              </section>
            ))}
      ...
      >
        <p
          className="text-center text-label text-gold mb-8"
        ...
        >
          {[...PDF_MENUS, DRINKS_PDF].map(menu => (
            <div
              key={menu.title}
              style={{ border: '1px solid #2A2A2A', overflow: 'hidden', background: '#0E0E0E' }}
            >
              {/* Mobile */}
              <a
                href={menu.pdf}
              ...
              >
                ...
                <a
                  ...
                  onMouseEnter={e => {
                    ...
                    a.style.color = '#A8956A'
                    a.style.borderColor = '#A8956A'
                  }}
                  onMouseLeave={e => {
                    const a = e.currentTarget as HTMLAnchorElement
                    a.style.color = '#F5F5F0'
                    a.style.borderColor = '#6B6B6B'
                  }}
                >
                  {m.openButton}
                </a>
              ...
              <div
                ...
                style={{ height: 420, overflow: 'hidden', background: '#111' }}
              >
                <iframe
